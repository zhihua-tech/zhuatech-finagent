...
@Bean CommandLineRunner seed(OperatingUnitRepository units,WorkRecordRepository records,ResourceRegisterRepository resources,ReviewRecordRepository reviews,UserRepository users,PasswordEncoder encoder){return args->{if(units.count()>0)return;
 ...
 reviews.saveAll(List.of(new ReviewRecord("CHECK-260801-014",a,"月结完整性",32,2,ReviewRecord.Result.PENDING,"程璟"),new ReviewRecord("CHECK-260801-009",b,"对账准确性",28,0,ReviewRecord.Result.PASSED,"苏禾"),new ReviewRecord("CHECK-260731-036",c,"费用政策命中",24,4,ReviewRecord.Result.FAILED,"宋惟")));
 String demo=encoder.encode("Demo@2026"); users.saveAll(List.of(new UserAccount("operator",demo,"苏禾",UserAccount.Role.DOMAIN_USER,"FIN-LEDGER"),new UserAccount("planner",demo,"程璟",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"宋惟",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}}
